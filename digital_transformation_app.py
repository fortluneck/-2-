...
if 'theme' not in st.session_state:
    st.session_state.theme = 'light'  # 默认亮色主题
...
with col2:
    # 主题切换按钮
    theme_button = st.button("🌙 切换至暗色模式" if st.session_state.theme == 'light' else "☀️ 切换至亮色模式", key="theme_switch")
    if theme_button:
...
# 构建文件路径
file_path = os.path.join('d:\\', '新建文件夹 - 副本 (2)', '生成app资料', '1999-2023年数字化转型指数与行业合并表.xlsx')
...
if df is not None:
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            filtered_df.to_excel(writer, index=False, sheet_name='数字化转型指数')
        
        excel_data = output.getvalue()
        st.download_button(
            label="💾 下载Excel格式数据",
            data=excel_data,
            file_name="数字化转型指数数据.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
    except Exception as e:
        st.write(f"Excel格式下载功能暂时不可用：{e}")
    
    # 图表分析
    st.header("📊 数据分析")
    
    # 1. 年份趋势分析
    st.subheader("1. 年份趋势分析")
    year_trend = filtered_df.groupby('年份')['数字化转型指数(0-100分)'].mean().reset_index()
    
    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(year_trend['年份'], year_trend['数字化转型指数(0-100分)'], marker='o', linewidth=2, markersize=8)
    ax.set_title('数字化转型指数年份趋势', fontsize=16)
    ...
    # 添加雷达图展示
    st.subheader("3.1 技术维度雷达图分析")
    
    # 准备雷达图数据
    radar_data = tech_avg.set_index('技术维度')['平均得分'].to_dict()
    ...
    try:
        ...
        # 如果数据量足够，还可以添加不同年份的对比雷达图
        if len(selected_years) > 1:
            st.subheader("3.2 不同年份技术维度对比")
            
            # 按年份分组计算各技术维度的平均值
            year_tech_avg = filtered_df.groupby('年份')[tech_dimensions].mean().reset_index()
            
            # 创建对比雷达图
            fig = go.Figure()
            
            # 为每个年份添加一个雷达图轨迹
            for _, row in year_tech_avg.iterrows():
                fig.add_trace(go.Scatterpolar(
                    r=row[tech_dimensions].values.tolist(),
                    theta=tech_dimensions,
                    fill='toself',
                    name=f'{row["年份"]}年',
                    hovertemplate='%{theta}: %{r:.2f}<extra></extra>'
                ))
            ...
            fig.update_layout(
                ...
                height=600,
                margin=dict(r=20, l=20, t=50, b=20),
                legend=dict(title='年份', orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1),
                # 添加导出按钮配置
                updatemenus=[{
                    'type': 'buttons',
                    'direction': 'left',
        ...
        st.write(f"雷达图生成错误：{e}")
        st.write("如果雷达图无法显示，可能是由于plotly库版本或配置问题。")
    
    # 4. 数字化转型指数分布
    st.subheader("4. 数字化转型指数分布")
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.histplot(filtered_df['数字化转型指数(0-100分)'], bins=20, kde=True, ax=ax)
    ax.set_title('数字化转型指数分布', fontsize=16)
    ...
    try:
        ...
        # 添加交互式标记
        for province, row in province_data.iterrows():
